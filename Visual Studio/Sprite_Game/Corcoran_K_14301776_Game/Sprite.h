...
class Sprite
{
	...
	public: void Sprite::specific_frame(int);
	public: void Sprite::advance_projectile2();   // Advance movement of sprite 
...
// Turn on and onfigure OpenGL window to suit work with sprites
...
void Sprite::specific_frame(int f)
{
	frame_no =f;
}
...
void Sprite::advance_projectile()
{
	...
	float g=0.05;    // acceleration due to gravity
...
void Sprite::advance_projectile2()//my own method for the beam projectile
{	
	float dt=0.6;    // time step
	float k1=0.15;  // air resistance
	float g=0.05;    // acceleration due to gravity

	/*ternary operator here - somewhat abandoned. the idea was to prevent 
	the beam moving completely over the asteroid and thus never colliding 
	despite it being in the line of fire .........works well anyway :) */

	sprite_y_position+=(sprite_y_velocity*dt)?sprite_y_velocity*dt:60; 
	sprite_y_velocity+=((g+(k1*sprite_y_position))*dt);  // Gravity and drag
}
