...
namespace OpenGLForm
{
    ...
    public System::Windows::Forms::NativeWindow
	{
		...
		GLMmodel *objmodel_M_O;
		GLMmodel *objmodel_M_M;
		GLMmodel *objmodel_M_I;
            ...
            GLsizei iWidth, GLsizei iHeight)
		{
			...
			cp->Parent = parentForm->Handle;
			 ...
			 // and make OpenGL compliant (no clipping)
				...
				//remove the "_smooth" for a more triangulated face
				objmodel_M_O = glmReadOBJ("mobius_O_smooth.obj");
				glmUnitize(objmodel_M_O);
				glmFacetNormals(objmodel_M_O);
				glmVertexNormals(objmodel_M_O, 90.0,GL_FALSE);

				objmodel_M_M = glmReadOBJ("mobius_M_smooth.obj");
				glmUnitize(objmodel_M_M);
				glmFacetNormals(objmodel_M_M);
				glmVertexNormals(objmodel_M_M, 90.0,GL_FALSE);

				objmodel_M_I = glmReadOBJ("mobius_I_smooth.obj");
				glmUnitize(objmodel_M_I);
				glmFacetNormals(objmodel_M_I);
				glmVertexNormals(objmodel_M_I, 90.0,GL_FALSE);
		...
		virtual System::Void Render(System::Void)
		{
			...
			glmDraw(objmodel_M_O,  GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL );  // sun
			...
			glRotatef(rotation,0,1,0);
			glmDraw(objmodel_M_M,  GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL ); // earth
			glPopMatrix();

			glRotatef(rotation/*-rotation+(rotation*0.5)*/,0,0,1);  // Moon about the earth
		//	glTranslatef(1.f, 0.0f, 0.0f);
			glScalef(0.35, 0.35, 0.35);
			glRotatef(0,0,1,0);
			glmDraw(objmodel_M_I,GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL ); // moon
